...
Singleton {
    ...
    // Usage counts shared by the launcher's relevance ranking
    // (rofi-style history). Keys look like "app:firefox.desktop",
    // "bin:btop" or "cmd:btop --version". Storage is one JSON object
    // per line; counts live in memory and the file is append-mostly
    // with an occasional full rewrite past 1000 raw lines.
    property var counts: ({})
    ...
    function record(key: string): void {
        const e = launchHistory.counts[key] ?? { c: 0, t: 0 }
    ...
    function pumpWrites(): void {
        ...
        // Cap batch size so a long offline queue can't blow ARG_MAX.
        ...
        writeProbe.command = [
            ...
            "qs",
            launchHistory.activeBatch.join("\n"),
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                // Keep the batch: prepend it so nothing is lost.
                // Don't loop immediately; the next record() retries.
                launchHistory.retryQueue =
                    launchHistory.activeBatch.concat(launchHistory.retryQueue)
